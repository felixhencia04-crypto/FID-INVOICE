...
async function test(secretSuffix) {
  const CLIENT_ID = 'BRN-0229-1783394866076';
  const SECRET_KEY = 'SK-c1pC2u9lDrwLpYdVz05v' + secretSuffix;
  ...
  try {
    const res = await axios.post('https://api-sandbox.doku.com' + targetPath, payload, {
      headers: {
        'Client-Id': CLIENT_ID, 'Request-Id': requestId,
        'Request-Timestamp': timestamp, 'Signature': signature
      }
    });
    console.log("SUCCESS with suffix:", secretSuffix);
    return true;
  } catch (err) {
    return false;
  }
...
async function runAll() {
  ...
  // I won't brute force 64 chars right now unless needed
}
runAll();
